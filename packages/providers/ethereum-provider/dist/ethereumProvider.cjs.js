...
;// CONCATENATED MODULE: external "@web3auth-mpc/base-provider"
const base_provider_namespaceObject = require("@web3auth-mpc/base-provider");
...
async function signingUtils_signTx(txParams, sign, txFormatter) {
  const finalTxParams = await txFormatter.formatTransaction(txParams);
  const common = await txFormatter.getCommonConfiguration();
  const unsignedEthTx = tx_namespaceObject.TransactionFactory.fromTxData(finalTxParams, {
    common
...
function signingUtils_getProviderHandlers(_ref) {
  let {
    txFormatter,
    sign,
    getPublic,
  ...
  return {
    ...
    processTransaction: async (txParams, _) => {
      ...
      const serializedTxn = await signingUtils_signTx(txParams, sign, txFormatter);
      const txHash = await providerEngineProxy.request({
        method: "eth_sendRawTransaction",
...
var EthereumSigningProvider_class;
...
class EthereumSigningProvider extends base_provider_namespaceObject.BaseProvider {
  ...
  async setupProvider(_ref2) {
    ...
    const providerHandlers = signingUtils_getProviderHandlers({
      txFormatter,
      sign,
      getPublic,
      getProviderEngineProxy: this.getProviderEngineProxy.bind(this)
...
EthereumSigningProvider_class = EthereumSigningProvider;
defineProperty_default()(EthereumSigningProvider, "getProviderInstance", async params => {
  const providerFactory = new EthereumSigningProvider_class({
    config: {
      chainConfig: params.chainConfig
    }
  });
